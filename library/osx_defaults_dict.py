#!/usr/bin/python
# -*- coding: utf-8 -*-
### !!! Copied from https://github.com/ansible/ansible/pull/24032 and updated to match
### !!! https://github.com/ansible-collections/community.general/blob/2.5.1/plugins/modules/system/osx_defaults.py
...
DOCUMENTATION = r"""
---
module: osx_defaults_dict
author:
# DO NOT RE-ADD GITHUB HANDLE!
- Franck Nijhof (!UNKNOWN)
short_description: Manage macOS user defaults
description:
  - osx_defaults_dict allows users to read, write, and delete macOS user defaults from Ansible scripts.
...
options:
  ...
  type:
    ...
    type: str
    choices: [ array, bool, boolean, date, dict, dictionary, float, int, integer, string ]
    default: string
  array_add:
    description:
...
class OSXDefaults(object):
        ...
        if data_type == "string":
            ...
            try:
                return datetime.strptime(
                    value.split("+")[0].strip(), "%Y-%m-%d %H:%M:%S"
                )
            except ValueError:
                raise OSXDefaultsException(
                    "Invalid date value: {0}. Required format yyy-mm-dd hh:mm:ss.".format(
                        repr(value)
                    )
                )
            ...
            try:
                ...
                raise OSXDefaultsException(
                    "Invalid float value: {0}".format(repr(value))
                )
            return value
        elif data_type == "array":
            if not isinstance(value, list):
                raise OSXDefaultsException(
                    "Invalid value. Expected value to be an array"
                )
            return value
            ...
            if isinstance(value, dict):
                ...
                raise OSXDefaultsException(
                    "Invalid dict value: {0}".format(repr(value))
                )
    ...
    def _convert_defaults_str_to_dict(value):
        """ Converts dict output from defaults to a dict """
        # Split output of defaults. Every line contains a value
    ...
    def read(self):
        ...
        rc, out, err = self.module.run_command(
            self._base_command() + ["read-type", self.domain, self.key]
        )
        ...
        if rc != 0:
            raise OSXDefaultsException(
                "An error occurred while reading key type from defaults: %s" % out
            )
        ...
        rc, out, err = self.module.run_command(
            self._base_command() + ["read", self.domain, self.key]
        )
        ...
        if rc != 0:
            raise OSXDefaultsException(
                "An error occurred while reading key value from defaults: %s" % out
            )
    ...
    def write(self):
        ...
        if self.type in ["dict", "dictionary"]:
            ...
            for new_value in value:
                dict_values.append(new_value["key"])
                dict_values.append("-" + new_value["type"])
                dict_values.append(str(new_value["value"]))
            value = dict_values

        rc, out, err = self.module.run_command(
            self._base_command()
            + ["write", self.domain, self.key, "-" + self.type]
            + value
        )

        if rc != 0:
            raise OSXDefaultsException(
                "An error occurred while writing value to defaults: %s" % out
            )

    def delete(self):
        """ Deletes defaults key from domain """
        rc, out, err = self.module.run_command(
            self._base_command() + ["delete", self.domain, self.key]
        )
        if rc != 0:
            raise OSXDefaultsException(
                "An error occurred while deleting key from defaults: %s" % out
            )
    ...
    def run(self):
        ...
        if (
            ...
            and not isinstance(self.current_value, value_type)
            and not isinstance(self.current_value, dict)
        ):
            raise OSXDefaultsException(
                "Type mismatch. Type in defaults: %s"
                % type(self.current_value).__name__
            )
        ...
        if (
            ...
            self.type in ["dict", "dictionary"]
            and self.current_value is not None
            and not self.dict_add
            and self.current_value == comparable_value
        ):
            return False
        elif (
            self.type in ["dict", "dictionary"]
            and self.current_value is not None
            and self.dict_add
            and set(comparable_value.items()).issubset(set(self.current_value.items()))
        ):
            return False
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            value=dict(type="raw"),
            state=dict(
                type="str", default="present", choices=["absent", "list", "present"]
            ),
